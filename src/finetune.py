    ...
    if run_validation:
        ...
        with torch.no_grad():
            for _ in range(valid_steps_per_epoch):
                if len(args.devices) == 1:
                    loss = _compute_mse_on_batch(layer, valid_batch_iterators[0], dtype=args.finetune_dtype, **kwargs)
                else:
                ...
                for _ in range(valid_steps_per_epoch):
                    if len(args.devices) == 1:
                        loss = _compute_mse_on_batch(
                            layer, valid_batch_iterators[0], dtype=args.finetune_dtype, **kwargs
                        )
                    else:
                        loss = _compute_mse_parallel(
                            args.devices,
