...
class StraightThroughAdamW(ConfigurableAdamW):
    ...
    def _aggregate_dequantized_weights(self):
        ...
        aggregated_params_by_name = {name: param.view(self.quantized_weights_by_name[name].shape)
        ...
        if destination_rank == own_rank:
            ...
            for i in range(world_size):
                if shard_sizes[i] == 0:
                    ...
                    async_ops.append(torch.distributed.recv(gather_buffers[i], src=i))
                else:
            ...
            async_ops.append(torch.distributed.send(shard, destination_rank))
