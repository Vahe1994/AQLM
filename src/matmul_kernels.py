...
@triton.autotune(
    configs=[
        ...
        for num_stages in (1, 2, 3, 4, 5) for num_warps in (1, 2, 4, 8)
    ],
    key=['in_features', 'out_features', 
         'num_codebooks', 'codebook_size', 'out_group_size', 'in_group_size',
         'num_input_groups', 'num_input_groups_next_power_of_2', 'compute_in_fp32'],
)
@triton.jit
def _aqlm_gemv_simple(
    input_vec_ptr, output_vec_ptr, codes_i16_ptr, codebooks_ptr, scales_ptr,
    
    in_features: tl.constexpr, out_features: tl.constexpr, 
    num_codebooks: tl.constexpr, codebook_size: tl.constexpr,
    out_group_size: tl.constexpr, in_group_size: tl.constexpr,
    num_input_groups: tl.constexpr, num_input_groups_next_power_of_2: tl.constexpr,
    compute_in_fp32: tl.constexpr, UNUSED: tl.constexpr,
):
    # variables ending with "_i" mean "for i-th output unit"
    pid = tl.program_id(axis=0)  # [0, 1, ... {out_features-1}]
    
    # Stage 1: load input data
    input_vec = tl.load(input_vec_ptr 
    ...
    codes_i_ptrs = (codes_i16_ptr + pid * num_input_groups * num_codebooks
    ...
    if out_group_size == 1:
        ...
        tl.store(output_vec_ptr + pid * out_group_size + tl.arange(0, out_group_size),
...
def aqlm_gemv_simple(
    input_vec: torch.Tensor, codes_i16: torch.ShortTensor, codebooks: torch.Tensor, scales: torch.Tensor,
    ...
    _aqlm_gemv_simple[grid](
        input_vec, output_vec, codes_i16, codebooks, scales,
        in_features, out_features, num_codebooks, codebook_size, out_group_size, in_group_size,
        num_input_groups, next_power_of_2(num_input_groups), compute_in_fp32,
    )
