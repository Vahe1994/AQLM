...
def finetune_head(
    *,
    ...
    ref_inputs: Sequence[torch.Tensor],
    args: Namespace,
    verbose: bool = True,
    **kwargs,
) -> nn.Module:
    ...
    assert isinstance(inputs, (list, tuple)) and isinstance(ref_inputs, (list, tuple))
    assert len(inputs) == len(args.devices)
    for i in range(len(args.devices)):
        assert isinstance(inputs[i], torch.Tensor) and isinstance(ref_inputs[i], torch.Tensor)
        if not args.offload_activations:
            assert inputs[i].device == ref_inputs[i].device == args.devices[i], (inputs[i].device, ref_inputs[i].device, args.devices)
        else:
            assert inputs[i].device == ref_inputs[i].device == torch.device("cpu")
            assert inputs[i].is_pinned() and ref_inputs[i].is_pinned()
    ...
    if len(args.devices) > 1:
        ...
        replicas[0] = layer
        reference_layer_replicas = torch.nn.parallel.replicate(reference_layer, args.devices, detach=True)
        kwargs_by_device = []
    ...
    batch_iterators = [
        iterate_minibatches(inputs[i], ref_inputs[i], batch_size=local_batch_size, device=args.devices[i])
        for i in range(len(args.devices))
    ]
    ...
    for epoch in range(args.finetune_max_epochs):
        ...
        for step in range(steps_per_epoch):
            if len(args.devices) == 1:
                loss = _compute_mse_on_batch(layer, reference_layer, batch_iterators[0], **kwargs)
            else:
                loss = _compute_mse_parallel(
                    args.devices,
                    replicas,
                    reference_layer_replicas,
...
def _compute_mse_on_batch(
    layer: nn.Module, reference_layer: nn.Module, batch_iter: Iterator[Tuple[torch.Tensor, torch.Tensor]], **kwargs
) -> torch.Tensor:
    """
    Compute the activation MSE error between transformer layers
    :param
    """
    inputs_batch, ref_inputs_batch = next(batch_iter)
    inputs_batch = inputs_batch.to(dtype=torch.float32)
    ref_inputs_batch = ref_inputs_batch.to(dtype=torch.float32)
    ...
    predictions = layer(inputs_batch, **kwargs)
    with torch.no_grad():
        targets = reference_layer(ref_inputs_batch, **kwargs)
    # loss = KL(softmax(targets) || softmax(predictions))
    return (torch.softmax(targets, dim=-1) * (torch.log_softmax(targets, dim=-1)
...
def _compute_mse_parallel(
    ...
    reference_layer_replicas: Sequence[nn.Module],
