...
for model, layers in MODELS.items():
    ...
    for K, M in layers:
        ...
        codebook_cuda.code1x16_matvec(A, B, C, codebook)
        # codebook_cuda.code2x8_matvec(A, B, C, codebook)
        print(torch.mean(torch.abs(C - C_ref)) / torch.mean(torch.abs(C_ref)))

        dense += benchmark(lambda: torch.matmul(A_ref, B, out=C))
        quant += benchmark(lambda: codebook_cuda.code1x16_matvec(A, B, C, codebook))
